...
ALLOWED_USERS = set(map(int, ALLOWED_USERS_RAW.split(","))) if ALLOWED_USERS_RAW.strip() else set()
...
# --- Маршрут для проверки состояния (Uptime Robot) ---
...
# Добавляем маршрут для проверки состояния, доступный по GET-запросу
